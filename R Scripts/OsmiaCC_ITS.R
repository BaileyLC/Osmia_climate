...
# Import data
  seqtab.nochim <- readRDS("OsmiaCC_seqsITSall.rds")
  taxa <- readRDS("OsmiaCC_taxaITSall.rds")
  metaITS_CC <- read.csv("OsmiaCC_master - ITS_all.csv")

## Create phyloseq object ----

# Re-create your df
  samples.out <- rownames(seqtab.nochim)
  samples <- data.frame(metaITS_CC)
...
# Sort data by Family
  y4 <- tax_glom(rareps, taxrank = 'Family') # agglomerate taxa
  y5 <- transform_sample_counts(y4, function(x) x/sum(x))
  y6 <- psmelt(y5)
  y6$Family <- as.character(y6$Family)
  y6$Family[y6$Abundance < 0.01] <- "Family < 1% abund."
  write.csv(y6, file = "y6.csv")
  y6$Family <- as.factor(y6$Family)
  head(y6)
...
# Plot Family by treatment  
  ggplot(data = y6, aes(x = combo_treat, y = Abundance, fill = Family)) + 
    geom_bar(stat = "identity", 
             position = "fill") + 
...
# Plot Family for each sample
  ggplot(data = y6, aes(x = sampleID, y = Abundance, fill = Family)) + 
    geom_bar(stat = "identity", 
             position = "fill") + 
    scale_fill_manual(values = colors) + 
...
# Sort data by Genus
  y7 <- tax_glom(rareps, taxrank = 'Genus') # agglomerate taxa
...
# Plot Genus by treatment
  ggplot(data = y9, aes(x = combo_treat, y = Abundance, fill = Genus)) + 
    geom_bar(stat = "identity", 
             position = "fill") + 
    scale_fill_manual(values = colors) + 
    theme(legend.position = "right") +
