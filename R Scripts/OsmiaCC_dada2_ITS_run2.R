...
### Purpose: Quality control of ITS rRNA data (run 2) using the DADA2 pipeline
...
# State path to cutadapt
  cutadapt <- "/Users/baileycrowley/miniconda3/bin/cutadapt"

# Run shell commands from R
  system2(cutadapt, args = "--version")

# Create output fileneames for the cutadapt-ed files and define the parameters
...
# Quality filtering and trimming
  out <- filterAndTrim(cutFs, filtFs, cutRs, filtRs,  # specify object that contains your unfiltered reads, followed by the object that contains your unfiltered reads
                      truncLen = c(220, 200), # truncates reads a specified base (forward, reverse) 
...
## Save output ---- 
  saveRDS(seqtab.nochim, file = "OsmiaCC_seqsITS_run2.rds")
  saveRDS(taxa, file = "OsmiaCC_taxaITS_run2.rds")
